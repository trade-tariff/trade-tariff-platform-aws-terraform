...
| [aws_wafv2_web_acl_rule.header_allow_multi](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/wafv2_web_acl_rule) | resource |
...
| <a name="input_header_allow_multi_rules"></a> [header\_allow\_multi\_rules](#input\_header\_allow\_multi\_rules) | Non-sensitive metadata for header-allow rules that accept any one of several possible values (e.g. any currently valid API key), such as name, priority, header\_name. Sensitive values are provided via the header\_allow\_multi\_values variable. | <pre>list(object({<br/>    name        = string<br/>    priority    = number<br/>    header_name = string<br/>  }))</pre> | `[]` | no |
| <a name="input_header_allow_multi_values"></a> [header\_allow\_multi\_values](#input\_header\_allow\_multi\_values) | Sensitive lists of acceptable header values, keyed by rule name. A request is allowed if the header exactly matches any one value in the list. | `map(list(string))` | `{}` | no |
