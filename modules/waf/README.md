...
| <a name="input_assets_rate_based_rule"></a> [assets\_rate\_based\_rule](#input\_assets\_rate\_based\_rule) | A rate-based rule specifically for assets defined in regex pattern set | <pre>object({<br/>    name      = string<br/>    priority  = number<br/>    rpm_limit = number<br/>    action    = string<br/>    custom_response = object({<br/>      response_code = number<br/>      body_key      = string<br/>      response_header = object({<br/>        name  = string<br/>        value = string<br/>      })<br/>    })<br/>  })</pre> | `null` | no |
...
| <a name="input_query_string_size_constraint_rules"></a> [query\_string\_size\_constraint\_rules](#input\_query\_string\_size\_constraint\_rules) | Custom query string size constraint rules | <pre>list(object({<br/>    name                    = string<br/>    priority                = number<br/>    action                  = string # allow | block | count<br/>    comparison_operator     = string # EQ | NE | LE | LT | GE | GT<br/>    size                    = number<br/>    excluded_uri_path_exact = optional(string)<br/>  }))</pre> | `[]` | no |
