...
| <a name="input_header_allow_rules"></a> [header\_allow\_rules](#input\_header\_allow\_rules) | Rules that allow requests presenting an exact header name/value pair. Evaluated before rate limiting — use for trusted internal clients. | <pre>list(object({<br/>    name         = string<br/>    priority     = number<br/>    header_name  = string<br/>    header_value = string<br/>  }))</pre> | `[]` | no |
